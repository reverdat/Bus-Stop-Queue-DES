...
def check_distributions():
    ...
    for filename in files:
        ...
        # 1. Truncated Exponential (Professor's Formula)
        if "trunc" in name:
            K = 30
            # Define the CDF manually using your formula
            # CDF = (e^2 - e^(2(1-c/K))) / (e^2 - 1)
            def trunc_cdf(c):
                # Clamp c between 0 and K to avoid math errors
                c = np.clip(c, 0, K)
                return (np.exp(2) - np.exp(2 * (1 - c/K))) / (np.exp(2) - 1)
            
            # PDF for plotting (Optional, for visual check)
        ...
        # 2. Erlang (Gamma in Python)
        # Note: Scipy's gamma takes 'a' (shape/k) and 'scale' (1/lambda)
        ...
        # 4. Hypoexponential (Sum of Exponentials)
        elif "hypo" in name:
            rates = [3, 7] 
            
            # Strategy: Simulate "Ground Truth" in Python and compare (2-sample KS)
            # This avoids writing the complex hypo PDF formula
            sim_size = len(data)
            r1 = np.random.exponential(1/rates[0], sim_size)
            r2 = np.random.exponential(1/rates[1], sim_size)
            ground_truth = r1 + r2 
            
            # Plot Density of ground truth
            kde = stats.gaussian_kde(ground_truth)
            plt.plot(x_space, kde(x_space), 'g--', lw=2, label='Python Simulation')
            
            # Test: Compare Zig Data vs Python Data
